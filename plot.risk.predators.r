plot.risk.predators<-function(mst.plaus=1,mlt.plaus=1,nst.plaus=1,nlt.plaus=1,Aspect=1,Xlimits=c(-0.1,1.35),Ylimits=c(-0.1,1.1)){
  ## George Watters
...
#  f.data<-rbind(f1,f7)
  ...
  #plot on gamma, grouped by predator (pred), using whole dataframe - updated to run x ssmu ESK 8.26.2016
    xyplot(prob~gamma|ssmu*option,groups=pred,data=new.data,type="l", panel=function(x,y,...){
  ...
  par.settings=list(superpose.line=list(col=c("black","blue","red","green"),lty=c(1,1,1,1))),
  main=list(label="48.1 at 45%, seals (red), pengs (blue), whales (green), fish (black)",cex=0.8,fontface="plain"),
  # strip=strip.custom(style=1,bg="transparent"),
  strip=strip.custom(style=1, bg="transparent",strip.levels = rep(TRUE,16)),
   # strip=strip.custom(style=1, bg="transparent",var.name = opn, factor.levels=mynames, strip.levels = rep(TRUE,15)),
